...
static void fw_update(void *device_data)
{
	...
	if (ssd->factory_cmd_param[0] == 0)
		rc = upgrade_firmware_from_platform(ssd->dev, true);
	else
...
static void print_raw_data_log(struct cyttsp4_samsung_sysfs_data* ssd,
					u8 *buf)
{
	int i, j;
	const u8 x_num = ssd->si->si_ptrs.pcfg->electrodes_x;
	const u8 y_num = ssd->si->si_ptrs.pcfg->electrodes_y;

	dev_info(ssd->dev, "%s: %d,%d\n", __func__, x_num, y_num);
	for (i = 0; i < x_num; i++) {
		pr_info("%s: [%02d]", ssd->dev->driver->name, i);
...
exit:
	if (rc == 0) {
		...
		print_raw_data_log(ssd, ssd->screen_buf + CY_CMD_RET_PNL_OUT_DATA_OFFS);
...
static inline int node_offset(struct cyttsp4_samsung_sysfs_data *ssd,
	int x, int y)
{
	return		ssd->factory_cmd_param[1] +
...
static void get_raw_diff(struct cyttsp4_samsung_sysfs_data* ssd)
{
	...
	if ((ssd->factory_cmd_param[0] < 0) ||
		(ssd->factory_cmd_param[0] >= ssd->si->si_ptrs.pcfg->electrodes_x) ||
		(ssd->factory_cmd_param[1] < 0) ||
		(ssd->factory_cmd_param[1] >= ssd->si->si_ptrs.pcfg->electrodes_y)) {
		...
		u16 node = node_offset(ssd, ssd->factory_cmd_param[0],
			ssd->factory_cmd_param[1]);
		if (ssd->raw_diff_element_size == 2)
			value = node_value_s16(ssd->screen_buf + CY_CMD_RET_PNL_OUT_DATA_OFFS,
				node);
		else
			value = ssd->screen_buf[CY_CMD_RET_PNL_OUT_DATA_OFFS + node];
		...
		dev_info(ssd->dev, "%s: node %d[%d,%d] = %d\n",
					__func__, node, ssd->factory_cmd_param[0],
					ssd->factory_cmd_param[1], value);
	}
...
static void print_idac_log(struct cyttsp4_samsung_sysfs_data* ssd,
					u8 *buf)
{
	int i, j;
	const u8 x_num = ssd->si->si_ptrs.pcfg->electrodes_x;
...
exit:
	if (rc == 0) {
		...
		print_idac_log(ssd, ssd->screen_buf + CY_CMD_RET_PNL_OUT_DATA_OFFS + gidac_node_num(ssd));

		snprintf(strbuff, sizeof(strbuff), "%d,%d", // local min, local max
...
//-- sysfs factory
	...
	rc = sysfs_create_link(&ssd->dev_factory->kobj, &cd->md.input->dev.kobj, "input");
	if (rc < 0) {
		dev_err(ssd->dev,
