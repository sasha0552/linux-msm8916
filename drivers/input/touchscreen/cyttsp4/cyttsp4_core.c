...
static int cyttsp4_si_get_ddata(struct cyttsp4_core_data *cd)
{
	...
	if (rc < 0)
		...
		dev_info(cd->dev, "%s: tsp version:%02x%04x[%02x]\n", __func__,
			si->sti->hw_version,
			get_unaligned_be16(&si->sti->fw_versionh),
			si->sti->config_version);
		cd->md.fw_ver_ic = si->sti->fw_versionh;
#endif
	}
	return rc;
}
...
static void cyttsp4_watchdog_timer(struct timer_list *timer)
{
	struct cyttsp4_core_data *cd =
			container_of(timer, struct cyttsp4_core_data, cdata);
...
static int _cyttsp4_config_charger_status(struct cyttsp4_core_data *cd)
{
	u32 value;
	int rc = 0;
	...
	rc = cyttsp4_get_parameter(cd, CY_RAM_ID_CHARGER_STATUS, &value);
	if (rc < 0) {
		dev_err(cd->dev, "%s: get param failed.\n", __func__);
		goto exit;
	}

	if (value == cd->charger_status) {
		dev_dbg(cd->dev, "%s: charger status not changed\n", __func__);
		goto exit;
	}

	value = cd->charger_status;
	dev_info(cd->dev, "%s: charger connected = %d\n", __func__, value);

	rc = cyttsp4_set_parameter(cd, CY_RAM_ID_CHARGER_STATUS, 1, value);
	if (rc < 0)
		dev_err(cd->dev, "%s: set param failed.\n", __func__);

exit:
	return rc;
}

static int cyttsp4_config_charger_status(struct cyttsp4_core_data *cd)
{
	int rc = 0;
	...
	if (rc < 0) {
		dev_err(cd->dev, "%s: Error on request exclusive r=%d\n",
				__func__, rc);
		return rc;
	}

	rc = _cyttsp4_config_charger_status(cd);
	if (rc < 0)
		dev_err(cd->dev, "%s: _cyttsp4_config_charger_status fail r=%d\n",
			__func__, rc);

	if (release_exclusive(cd, cd->dev) < 0)
		dev_err(cd->dev, "%s: Error on release exclusive\n",
			__func__);

	return rc;
}

static void cyttsp4_charger_status_new(struct tsp_callbacks *cb, int connected)
{
	struct cyttsp4_core_data *cd =
			container_of(cb, struct cyttsp4_core_data, callbacks);

	dev_info(cd->dev, "%s: %d\n", __func__, connected);

	cd->charger_status = connected;

	if (cd->touch_stopped)
		return;

	cyttsp4_config_charger_status(cd);
}

static void cyttsp4_register_callback(struct tsp_callbacks *cb)
{
...
#ifdef USE_TSP_TA_CALLBACKS
	_cyttsp4_config_charger_status(cd);
#endif
	cyttsp4_start_wd_timer(cd);
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	rc = request_threaded_irq(cd->irq, /*cyttsp4_hard_irq*/NULL, cyttsp4_irq,
	...
	/* Store parent structure as member */
	cd->cdata = cd;
...
#ifdef USE_TSP_TA_CALLBACKS
	cd->register_cb = cyttsp4_register_callback;
	cd->callbacks.inform_charger = cyttsp4_charger_status_new;
	if (cd->register_cb)
		cd->register_cb(&cd->callbacks);
#endif
