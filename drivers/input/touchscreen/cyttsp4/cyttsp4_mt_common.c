	...
	for (i = 0; i < num_cur_rec; i++) {
		...
		if (sig != CY_IGNORE_VALUE) {
			...
			if (tch.abs[CY_TCH_E] == CY_EV_TOUCHDOWN) {
				...
				dev_info(dev, "[P][%d]\n", t);
#else
				dev_info(dev, "[P][%d] x=%d, y=%d\n",
					t, tch.abs[CY_TCH_X], tch.abs[CY_TCH_Y]);
#endif
#if defined(TSP_BOOSTER)
...
#ifdef CONFIG_SAMSUNG_PRODUCT_SHIP
				dev_info(dev, "[R][%d] mc=%d, ver=%02x\n", t,
					mt_count[t], md->fw_ver_ic);
#else
				dev_info(dev, "[R][%d] x=%d, y=%d, mc=%d, ver=%02x\n",
					t, tch.abs[CY_TCH_X], tch.abs[CY_TCH_Y],
					mt_count[t], md->fw_ver_ic);
#endif
				mt_count[t] = 0;
#ifdef CYTTSP4_MT_DEBUG_PRINT
